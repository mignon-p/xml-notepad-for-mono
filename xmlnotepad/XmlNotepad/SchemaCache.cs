...
namespace XmlNotepad
{
    ...
    public class SchemaCache : IXmlSerializable
    {
            ...
            if (st.Content is XmlSchemaSimpleTypeList) {
                ...
                foreach (XmlSchemaFacet f in cr.Facets) {
                    if (f is XmlSchemaEnumerationFacet) {
                        ...
                        list.Add(ef.Value, GetAnnotation(ef, SchemaCache.AnnotationNode.Tooltip));
                    }
                }                
            }
            return;
        ...
        public static XmlSchemaDocumentation GetDocumentation(XmlSchemaAnnotated a) {
        ...
        public static string GetAnnotation(XmlSchemaAnnotated a, AnnotationNode node) {
            XmlSchemaAnnotation ann = a.Annotation;
            if (ann == null) return null;
            string filter = node.ToString().ToLowerInvariant();
            if (filter == "default") filter = "";
            string result = GetMarkup(ann, filter);
            if (!string.IsNullOrEmpty(result)) return result;
            return GetMarkup(ann, null);
        }
